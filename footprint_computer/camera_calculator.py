...
# read the csv file with camera height, elevation in it
import pandas as pd
from geojson import Point, Polygon, Feature, FeatureCollection, dump

df = pd.read_csv("1815_37.csv")
# df = df[43:150]
ID = df["photo_ID"].tolist()
Cam_height = df["True_height"].tolist()
x = df["xcoord"].tolist()
y = df["ycoord"].tolist()
flight_direction = df["Azimuth_dd"].tolist()


f = open("footprints_1815_037.geojson", "a")
features = []
# getBoundingPolygon(FOVh, FOVv, altitude, roll, pitch, heading):
for n in range(len(df)):
    ...
    bbox = c.getBoundingPolygon(
                ...
                math.radians(60),
                Cam_height[n],
                math.radians(0),  # 0 for 1815, 300 for 0347
                math.radians(0),
                math.radians(338))  # 9 for 0347,  338 for 1815
    corners = []
    for i, p in enumerate(bbox):
        # print("point:", i, '-', p.x, p.y, p.z)
        poly = Polygon(())
        corners.append(tuple([p.x + x[n], p.y + y[n]]))
    corners.append(corners[0])
# print(corners)
# print(corners[3][1])

    features.append(Feature(geometry=Polygon([[(corners[0][0],corners[0][1]), (corners[1][0],corners[1][1]), (corners[2][0],corners[2][1]), (corners[3][0],corners[3][1]), (corners[0][0], corners[0][1]) ]])) )
# print(features)
